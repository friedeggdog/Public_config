...
sudo bash -c "echo 'alias genStudent=\"/usr/local/bin/genStudent.sh\"' >> /etc/bash.bashrc"

sudo bash -c "echo 'alias permit=\"/usr/local/bin/permit.sh\"' >> /etc/bash.bashrc"

sudo bash -c "echo 'alias updateDefaulter=\"/usr/local/bin/updateDefaulter.sh\"' | tee -a /home/{Opal,GarnetA,GarnetB,Agate}/.bashrc"

sudo bash -c "echo 'alias messAllocation=\"/usr/local/bin/messAllocation.sh\"' >> /home/HAD/.bashrc"
sudo bash -c "echo 'alias messAllocation=\"/usr/local/bin/messAllocation.sh\"' | tee -a /home/*/*/*/.bashrc"

sudo bash -c "echo 'alias feeBreakup=\"/usr/local/bin/feeBreakup.sh\"' | tee -a /home/*/*/*/.bashrc"

sudo bash -c "echo 'alias signOut=\"/usr/local/bin/signOut.sh\"' | tee -a /home/*/*/*/.bashrc"
...
then 
    ...
    sudo bash -c "echo '/usr/local/bin/signOut.sh 2>/dev/null' | tee -a /etc/bash.bashrc"
fi
