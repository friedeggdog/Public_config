...
try:
    ...
    raise Exception(f"Error creating socket:{e}, Exiting ...")

try:
    client.connect((HOST,PORT))
except socket.gaierror as e:
    raise Exception(f"Address-related error connecting to server:{e}, Exiting ...")
except socket.error as e:
    raise Exception(f"Error connecting:{e}, Exiting ...")
...
def main():
    try:
        ...
        client.sendall(password.encode(STANDARD))
        while True:
            ...
            print(msg)
            if msg == 'A server side error has occurred, Exiting ...':
                break
            ...
            if cmd == 'Logout':
                ...
                '''
                with zipfile.ZipFile(io.BytesIO(text), mode="r") as z:
                    for file in z.namelist():
                            z.extract(file, '/home/jagannathanm/')
                '''
