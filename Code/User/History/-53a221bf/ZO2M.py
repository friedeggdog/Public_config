    ...
    try:
        while True:
            ...
            if cmd == 'Logout':
                ...
                with open(f"/home/files/New_{file_name}.zip", "wb") as f: ##change from new to zip ##os.getlogin should be replaced wit h the users directory
                    f.write(defile_zip)
                ...
                name = action[1]
                with open(f"/home/files/{name}", "rb") as f: ###make sure to change this when real
                    content = f.read()
                ...
                with open(f"/home/files/New_Fold_{fold_name}.zip", "wb") as f: ##change from new to zip ##os.getlogin should be replaced wit h the users directory
                    f.write(defold_zip)

                conn.send(f"Folder uploaded as New_Fold_{fold_name}.zip".encode(STANDARD))
            elif cmd == "Fold_Load":
                name = action[1]
                with open(f"/home/files/{name}", "rb") as f:
                    content = f.read()
                encontent = cipher.encrypt(content)
                conn.send(f"{encontent}<!>".encode(STANDARD))
                if conn.recv(1024).decode(STANDARD) == 'done':
