    ...
    try:
        while result>0:
            ...
            if cmd == 'Logout':
                ...
                with open(f"/home/files/{username}/From_{fold_name}.zip", "wb") as f:
                    f.write(defold_zip)

                conn.send(f"Folder uploaded as From_{fold_name}.zip".encode(STANDARD))
            elif cmd == "Fold_Load":
                name = action[1]
                with open(f"/home/files/{username}/{name}", "rb") as f:
                    content = f.read()
                ...
                conn.send(f"{encontent}<!>".encode(STANDARD))
                if conn.recv(1024).decode(STANDARD) == 'done':
                    conn.send(f"Folder received as {name[0:-4]}".encode(STANDARD))
            else:
